	...
	if (! defined $markdown_sub) {
		...
		if (exists $config{multimarkdown} && $config{multimarkdown}) {
			...
			if ($@) {
				...
				debug(gettext("mdwn: using multimarkdown"));
				$markdown_sub=sub {
		...
		if (! defined $markdown_sub &&
			...
			if (! $@) {
				...
				debug(gettext("mdwn: using discount"));
				$markdown_sub=sub {
					my $t=shift;
		...
		if (! defined $markdown_sub) {
			...
			if (! $@) {
				debug(gettext("mdwn: using CommonMark"));
				$markdown_sub=sub {
		...
		if (! defined $markdown_sub) {
			...
			if (! $@) {
				if (Text::Markdown->can('markdown')) {
					debug(gettext("mdwn: using Text::Markdown::markdown"));
				...
				eval q{use Markdown};
				if (! $@) {
					debug(gettext("mdwn: using Markdown::Markdown"));
